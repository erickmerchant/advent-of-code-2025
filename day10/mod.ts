...
export async function part2(
  ...
  //     const targetStr = ln.match(/\{([0-9,]+)\}/);
  //     const buttonStrs = Array.from(ln.matchAll(/\(([0-9,]+)\)/g));

  //     let target: Array<number> = [];

  //     if (targetStr) {
  //       target = targetStr[1].split(",").map((s) => +s);
  //     }

  //     const buttons: Array<Array<number>> = [];
  ...
  // const results = [Promise.resolve(0)];
  // const size = 6;
  // let id = 0;
  // let r = 0;

  // function startWorker(machine: Machine2) {
  ...
  //   results.push(promise.then((n: number) => {
  //     console.log(++r);

  //     if (machines.length) {
  //       startWorker(machines.pop()!);
  //     }
  ...
  // for (const machine of machines.splice(0, size)) {
  //   startWorker(machine);
  // }

  // await Promise.all(results);

  // return (await Promise.all(results)).reduce((acc, curr) => acc + curr);
